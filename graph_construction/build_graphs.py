...
def construct_graph(
    ...
    edge_attr = np.stack(
        (
            edge_df.dr.values / feature_scale[0],
    ...
    y, n_corrected = correct_truth_labels(
        hits, edge_df[["index_1", "index_2"]], y, pid1
    )
    ...
    if np.sum(np.isnan(x)):
        logging.info("WARNING: x contains nan!")
    if np.sum(np.isnan(particle_id)):
        logging.info("WARNING: particle_id contains nan!")
...
def process_event(
    ...
    hits, particles = select_hits(
        hits, truth, particles, pt_min, endcaps, remove_noise, remove_duplicates
    )
    hits = hits.assign(evtid=evtid)

    # get truth information for each particle
    hits_by_particle = hits.groupby("particle_id")
    particle_properties = pd.read_csv(
        join(base_dir, f"particle_properties/{evtid}.csv"), header=0
    )
    ...
    track_props = particle_properties[
        ["particle_id", "pt", "d0", "q", "reconstructable"]
    ].values
    ...
    hits_sectors, sector_info = split_detector_sectors(
        hits, phi_edges, eta_edges, phi_overlap=phi_overlap, eta_overlaps=eta_overlaps
    )
    ...
    feature_scale = np.array(
        [1000.0, np.pi / n_phi_sectors, 1000.0, 1 / 1000.0, 1 / 1000.0]
    )
    ...
    if measurement_mode:
        ...
        output["summary_stats"] = {
            "efficiency": n_true / n_track_segs,
    ...
    filenames = [
        os.path.join(output_dir, f"{base_prefix}_s{i}") for i in range(len(sectors))
    ]

    logging.info("Event %i, writing graphs", evtid)
    if save_graphs:
        for sector, filename in zip(sectors, filenames):
            track_param_scale = np.array([1.0, 10**3, 1.0])
            track_params = np.array(
                [track_props[p][:3] / track_param_scale for p in sector["particle_id"]]
            )
            reconstructable = np.array(
                [track_props[p][3] for p in sector["particle_id"]]
            )
            np.savez(
                filename,
                **dict(
...
def main(args):
    ...
    n_nodes = np.array([graph_stats["n_nodes"] for graph_stats in summary_stats])
    n_edges = np.array([graph_stats["n_edges"] for graph_stats in summary_stats])
    purity = np.array([graph_stats["purity"] for graph_stats in summary_stats])
    efficiency = np.array([graph_stats["efficiency"] for graph_stats in summary_stats])
    boundary_fraction = np.array(
        [graph_stats["boundary_fraction"] for graph_stats in summary_stats]
    )
    logging.info(
        f"Events ({evtid_min},{evtid_max}), average stats:\n"
    ...
    # output to csv
    graph_level_stats = pd.DataFrame(
        {
            "n_nodes": n_nodes,
            "n_edges": n_edges,
            "purity": purity,
    ...
    sector_stats_list = [graph_stats["sector_stats"] for graph_stats in summary_stats]
    ...
    for s in range(num_sectors):
        ...
        logging.info(
            f"Event ({evtid_min},{evtid_max}), Sector {s}, average stats:\n"
