...
def map_pt(pt, to_str=True):
    num_to_str = {0: '0p0', 0.1: '0p1', 0.2: '0p2', 0.3: '0p3',
                  0.4: '0p4', 0.5: '0p5', 0.6: '0p6', 0.7: '0p7',
                  0.8: '0p8', 0.9: '0p9', 1: '1', 1.1: '1p1',
                  1.2: '1p2', 1.3: '1p3', 1.4: '1p4', 1.5: '1p5',
                  1.6: '1p6', 1.7: '1p7', 1.8: '1p8', 1.9: '1p9',
                  2.0: '2'}
    if to_str: return num_to_str[pt]
    str_to_num = {v: k for k, v in num_to_str.items()}
    return str_to_num[pt]

def get_file_prefixes(indir, trackml=True, evtid_min=0, evtid_max=8000, 
                      n_tasks=1, task=0, codalab=True):
    all_files = os.listdir(indir)
    suffix = '-hits.csv.gz'
    if not codalab: suffix = '-hits.csv'
    file_prefixes = sorted(join(indir, f.replace(suffix, ''))
...
def load_module_map(path):
    ...
    module_map = {key: item.astype(bool) 
                  for key, item in module_maps.items()}
    return module_map

def filter_file_prefixes(file_prefixes, outdir):
    existing_files = os.listdir(outdir)
    existing_evtids = [f.split('_')[0].split('00000')[-1]
    ...
    file_prefixes = [f for f in file_prefixes 
                     if f.split('00000')[-1] not in existing_evtids]
    logging.info('Skipping pre-existing graphs, calculating ' 
