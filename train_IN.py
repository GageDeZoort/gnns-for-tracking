...
def parse_args(args):
    ...
    parser.add_argument(
        "--model-outdir",
        type=str,
        default="train_output/models/EC1/",
        help="directory in which to save models",
    )
    parser.add_argument(
        "--stats-outdir",
        type=str,
        default="train_output/stats/EC1/",
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, fname) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            percent_complete = 100.0 * batch_idx / N
            logging.info(
                f"Train Epoch: {epoch} [{batch_idx}/{N}"
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step()
        if args.save_model:
            model_name = join(args.model_outdir, job_name + f"_epoch{epoch}")
