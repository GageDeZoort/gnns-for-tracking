...
job_name = "can_train1_ptmin1_q_sb_lr_job_10_epoch_40"
...
graph_paths = np.array([os.path.join(graph_indir, f) for f in graph_files])

eps_per_sector = {}
for s in range(n_sectors):
    print(f" --> Sector {s}")
    graphs = [f for f in graph_paths if int(f.split("_s")[-1].split(".")[0]) == s][
        0:100
    ]
    ...
    for eps in np.arange(0.1, 1.4, 0.05):
        ...
        with torch.no_grad():
            for batch, (data, f) in enumerate(test_loader):
                ...
                if torch.sum(torch.isnan(data.x)):
                    print("x has nan")
                if torch.sum(torch.isnan(data.edge_index)):
                    print("edge_index has nan")
                if torch.sum(torch.isnan(data.edge_attr)):
                    print("edge_attr has nan")
                if torch.sum(torch.isnan(data.y)):
                    print("y has nan")
