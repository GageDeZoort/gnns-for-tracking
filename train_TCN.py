...
def test(args, model, device, test_loader, thld=0.5):
    ...
    with torch.no_grad():
        for batch_idx, (data, f) in enumerate(test_loader):
            ...
            TP = torch.sum((data.y == 1).squeeze() & (w > thld).squeeze()).item()
            TN = torch.sum((data.y == 0).squeeze() & (w < thld).squeeze()).item()
            FP = torch.sum((data.y == 0).squeeze() & (w > thld).squeeze()).item()
            FN = torch.sum((data.y == 1).squeeze() & (w < thld).squeeze()).item()
            acc = zero_div(TP + TN, TP + TN + FP + FN)
            accs.append(acc)

            # edge weight loss
            loss = F.binary_cross_entropy(w.squeeze(1), data.y, reduction="mean").item()
            losses_w.append(loss)

            # condensation loss
            particle_id = data.particle_id
            loss_c = condensation_loss(
                beta, xc, particle_id, device=device, q_min=args.q_min
            ).item()
            loss_c *= args.loss_c_scale
            losses_c.append(loss_c)

            # background loss
            loss_b = background_loss(
                beta, xc, particle_id, device="cpu", q_min=args.q_min, sb=args.sb
            ).item()
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, f) in enumerate(train_loader):
        ...
        # background loss
        loss_b = background_loss(
            beta, xc, particle_id, device=device, q_min=args.q_min, sb=args.sb
        )
        loss_b *= args.loss_b_scale

        # object loss
        if args.predict_track_params:
            loss_o = object_loss(p, beta, track_params, particle_id, device=device)
        ...
        if batch_idx % args.log_interval == 0:
            ...
            logging.info(
                f"...losses: w={loss_w.item()}, c={loss_c.item()}"
...
def main(args):
    ...
    graph_paths = np.array(
        [os.path.join(args.indir, graph_file) for graph_file in graph_files]
    )
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        train_loss, tlw, tlc, tlb = train(
            args, model, device, train_loader, optimizer, epoch
        )
        thld = validate(model, device, val_loader)
        test_loss, te_lw, te_lc, te_lb, te_acc = test(
            args, model, device, test_loader, thld=thld
        )
        scheduler.step()
        ...
        stat_out = os.path.join(args.outdir, f"{args.stat_outfile}.csv")
